...
prisma/seed.ts                      realistic seed data (6 users, 10 dreams, ratings, comments, stars)
jest.config.ts
jest.setup.ts
.env.local.example
```
...
`★ Insight ─────────────────────────────────────`
The seed script bypasses the `awardStars()` helper and writes StarTransactions + totalStars directly via Prisma. This avoids Next.js App Router path alias (`@/`) issues in the seed context, and lets us compute all star totals upfront rather than triggering them through action chains.
`─────────────────────────────────────────────────`

- [ ] **Install `tsx` (TypeScript runner for the seed script)**

```bash
npm install --save-dev tsx
```

- [ ] **Add prisma seed config to `package.json`** (merge into existing object)
...
- [ ] **Create `prisma/seed.ts`** with all realistic data
...
✓ Created 6 users  (all passwords: password123)
✓ Created tags
✓ Created 10 dreams
✓ Created ratings and updated dream averages
✓ Created top-level comments
✓ Created comment replies
✓ Created star transactions
✓ Updated user star totals
✓ Set today's featured dream to Sky Island
...
- [ ] **Verify in Neon console** — check that all tables have rows: Users(6), Dreams(10), Tags(11), DreamTag(30), Ratings(50), Comments(26), StarTransactions(~86), Notifications(~86), FeaturedDream(1).

- [ ] **Commit**

```bash
git add prisma/seed.ts package.json
git commit -m "feat: database seed script with 6 users, 10 dreams, ratings, comments, and star economy"
```

---

## Task 4: NextAuth Configuration + Middleware
...
## Task 5: Pixel Theme + Root Layout
...
## Task 6: Auth Pages (Signup + Signin)
...
## Task 7: Utility Libraries (ranking, stars, featured, tags, dreamers)
...
## Task 8: Dashboard Layout (Sidenav + User Card)
...
## Task 11: Dream Detail — Rating + Threaded Comments
...
## Task 13: Activity (Notifications) Page
...
## Task 15: Profile Pages + Game Entry Point + Deployment Prep
